...
from loss.dense_loss import Loss_Doc
from models.dfdb.DFDB import GFBNet
...
parser.add_argument('--dataRoot', type=str, default=r'datasets/HDIBCO/img')
...
Do_data = DataLoader(Doc_data, batch_size=batchSize, shuffle=True, num_workers=args.numOfWorkers, drop_last=False,
                     pin_memory=True)
binarization = GFBNet(5)
discriminator = Discriminator(4)
loss_doc = Loss_Doc(binarization)
B_optimizer = optim.Adam(binarization.parameters(), lr=0.0003)
...
for epoch in range(1, num_epochs + 1):
    binarization.train()
    for num, (inputImg, ostu, sobel, gt, gt_Sobel, gray, prewitt, img_x, img_y, name) in enumerate(Do_data):
        if cuda:
            ...
            prewitt = prewitt.cuda()
        D_optimizer.zero_grad()
        bin_out, mid_outP2, mid_outP3, mid_outP5, mid_outP4 = binarization(inputImg, ostu, sobel)
        ...
        fin_l1_loss, fin_BCEloss, fin_dice_loss, fin_jaccard_loss, mid_loss = loss_doc(inputImg, bin_out, gt, mid_outP2,
                                                                                       mid_outP3, mid_outP4, mid_outP5)
        fake_preds = discriminator(bin_out, inputImg)
        adversarial_loss = torch.nn.BCELoss()(fake_preds, torch.ones_like(fake_preds))
        G_loss = 10 * fin_l1_loss + fin_BCEloss + fin_dice_loss + fin_jaccard_loss + mid_loss + 0.1 * adversarial_loss
        G_loss.backward()
        B_optimizer.step()
        print(
            '[{}/{}] Epoch {}: G Loss: {:.4f}, D Loss: {:.4f}, Mid Loss: {:.4f}, Adversarial Loss: {:.4f}, D_real_loss: {:.4f}, D_fake_loss: {:.4f}'.format(
                num, len(Doc_data) // batchSize, epoch, G_loss.item(), D_loss.item(),
                mid_loss.item(), adversarial_loss.item(), D_real_c.item(), D_fake_c.item())
        )
        ...
        if num % 50 == 0:
            ...
            cv_imwrite(
                ...
                outt)
    if epoch % 1 == 0:
        torch.save(binarization.state_dict(), args.modelsSavePath + '/Netself_{}.pth'.format(epoch))
