...
parser.add_argument('--dataRoot', type=str,
                    default='datasets/test/img', help='path for test data')
parser.add_argument('--pretrained', type=str, default='./checkSave/D2019/Netself_60.pth',
...
for param in netG.parameters():
    ...
    netG.eval()
for num, (inputImg, ostu, sobel, img_x, img_y, name,prewitt) in enumerate(Do_data):  # 只加载6张图片
    if cuda:
        inputImg = inputImg.cuda()
        ostu = ostu.cuda()
        sobel = sobel.cuda()
        prewitt= prewitt.cuda()
    mid_out = netG(inputImg, ostu, prewitt)
